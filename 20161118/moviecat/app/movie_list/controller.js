...
(function(angular) {
  ...
  module.controller('MovieListController', [
    ...
    function($scope, $route, $routeParams, HttpService) {
      ...
      HttpService.jsonp(
        'http://api.douban.com/v2/movie/' + $routeParams.category, { start: start, count: count,q:$routeParams.q },
        function(data) {
          $scope.title = data.title;
          $scope.subjects = data.subjects;   //详细
          $scope.totalCount = data.total;  //总数
          $scope.totalPages = Math.ceil($scope.totalCount / count);  //总的页数
          $scope.loading = false;
          $scope.$apply();  
